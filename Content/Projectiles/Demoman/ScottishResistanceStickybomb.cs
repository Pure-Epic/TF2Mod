...
{
    public class ScottishResistanceStickybomb : ModProjectile
    {
        public int Time
        {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            Main.player[Projectile.owner].GetModPlayer<Items.Demoman.ScottishResistancePlayer>().stickybombsReturned++;
        ...
        public override void OnHitPlayer(Player target, int damage, bool crit)
        {
            if (target == Main.player[Projectile.owner])
            {
                ...
                int selfDamage = Convert.ToInt32(Math.Floor(target.statLifeMax2 * 0.25f));
                if (p.gunboats)
                    selfDamage = Convert.ToInt32(Math.Floor(target.statLifeMax2 * 0.1f));
                ...
                {
                    airTime++;
                    velocityX = Utils.Clamp(velocityX, -10f, 10f);
                    velocityY = Utils.Clamp(velocityY, -10f, 10f);
