    ...
    public class Grenade : ModProjectile
    {
        ...
        public override void OnHitPlayer(Player target, int damage, bool crit)
        {
            if (target == Main.player[Projectile.owner])
            {
                ...
                int selfDamage = Convert.ToInt32(Math.Floor(target.statLifeMax2 * 0.2f));
                if (p.gunboats)
                    selfDamage = Convert.ToInt32(Math.Floor(target.statLifeMax2 * 0.08f));
                ...
                if (airTime <= maxAirTime)
                {
                    ...
                    target.velocity = new Vector2(-velocityX * 12.5f, -velocityY * 12.5f);
                }
                else
                {
