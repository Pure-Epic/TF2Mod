...
namespace TF2.Content.UI.HUD.Spy
{
    ...
    internal class DeadRingerChargeMeterHUD : TF2HUD
    {
        ...
        protected override void HUDPreInitialize(out UIElement _area, out UIImage _texture)
        {
            ...
            UIText _cloak = new UIText(TF2HUDSystem.TF2HUDLocalization[13], 0.5f)
            {
                HAlign = 0.5f,
                VAlign = 0.75f,
                IgnoresMouseInteraction = true
        ...
        protected override void HUDDraw(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.Draw(TextureAssets.MagicPixel.Value, new Rectangle(left, hitbox.Y, TF2.Round(hitbox.Width * feignDeathPlayer.cloakMeter / feignDeathPlayer.cloakMeterMax), hitbox.Height), Color.White);
        }
    }
}