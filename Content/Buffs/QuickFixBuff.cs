...
{
    public class QuickFixBuff : ModBuff
    {
        ...
        {
            npc.GetGlobalNPC<MediGunBuffNPC>().uberChargeBuff = true;
        ...
        {
            player.GetModPlayer<QuickFixBuffPlayer>().uberChargeBuff = true;
            for (int i = 0; i < BuffLoader.BuffCount; i++)
            {
    ...
    public class QuickFixBuffPlayer : MediGunBuffPlayer
    {
        public override void PostUpdate()
        {
            ...
            if (uberChargeBuff)
                SpawnDusts(Player);
            if (p.activateUberCharge && Player.HasBuff<QuickFixBuff>())
            {
                p.uberChargeTime++;
    ...
    public class QuickFixBuffNPC : GlobalNPC
    {
        public override bool InstancePerEntity => true;

        public override void ModifyHitNPC(NPC npc, NPC target, ref NPC.HitModifiers modifiers)
        {
            if (npc.HasBuff<QuickFixBuff>())
