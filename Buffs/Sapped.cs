...
namespace TF2.Buffs
{
    ...
    public class Sapped : ModBuff
    {
        ...
        {
            DisplayName.SetDefault("Sapped!"); // Buff display name
            Description.SetDefault("Slow and painful death"); // Buff description
        ...
        {
            player.GetModPlayer<SappedPlayer>().lifeRegenDebuff = true;
        }

        public override void Update(NPC npc, ref int buffIndex)
        {
            npc.GetGlobalNPC<SappedNPC>().lifeRegenDebuff = true;
        }
    }
        ...
        {           
            ...
            {   
                ...
                if (timer >= 60)
                {
                    ...
                    npc.HitEffect(0, (int)(25 * damageMultiplier));
                    npc.life -= (int)(25 * damageMultiplier);
                    npc.checkDead();
                    timer = 0;
                }
                int dustIndex = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.Electric, 0f, 0f, 100, default(Color), 3f);
