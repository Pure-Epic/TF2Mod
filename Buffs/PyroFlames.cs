...
namespace TF2.Buffs
{
    ...
    public class PyroFlames : ModBuff
    {
        ...
        {
            DisplayName.SetDefault("Afterburn"); // Buff display name
        ...
        {
            player.GetModPlayer<PyroFlamesPlayer>().lifeRegenDebuff = true;
        }

        public override void Update(NPC npc, ref int buffIndex)
        {
            npc.GetGlobalNPC<PyroFlamesNPC>().lifeRegenDebuff = true;
        }
    }
    ...
    public class PyroFlamesPlayer : ModPlayer
    {
        ...
        public override void UpdateBadLifeRegen()
        {
            if (lifeRegenDebuff)
            {
                ...
                //Player.statLife -= (int)(8 * damageMultiplier);
            }
            else
            {
                timer = 0;
    ...
    public class PyroFlamesNPC : GlobalNPC
    {
        ...
        public override void UpdateLifeRegen(NPC npc, ref int damage)
        {
            if (lifeRegenDebuff)
            {
                ...
                if (timer >= 30)
                {
                    npc.HitEffect(0, (int)(4 * damageMultiplier));
                    npc.life -= (int)(4 * damageMultiplier);
                    npc.checkDead();
                    timer = 0;
                }
                int dustIndex = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.Torch, 0f, 0f, 100, default(Color), 3f);
