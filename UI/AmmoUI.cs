...
namespace TF2.UI
{
	...
	internal class AmmoUI : UIState
	{
		...
		public override void Draw(SpriteBatch spriteBatch)
		{
			...
			if (!(Main.LocalPlayer.HeldItem.ModItem is TF2Weapon ) || Main.LocalPlayer.HeldItem.ModItem is TF2WeaponNoAmmo) //change to any weapon class if UI is weapon exculsive
				return;
			//WeaponSystem modWeapon = Main.LocalPlayer.GetModPlayer<WeaponSystem>();
			//if (modWeapon.ammoMax2 == 0) //only apply if the weapon has ammoclip
				//return;
		...
		{
			if (!(Main.LocalPlayer.HeldItem.ModItem is TF2Weapon))
				return;
